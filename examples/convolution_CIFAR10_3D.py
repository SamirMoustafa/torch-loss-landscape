...
if __name__ == "__main__":
    ...
    if exists(checkpoint_file_dir):
        model.load_state_dict(load(checkpoint_file_dir))
    else:
        ...
        save(model.state_dict(), checkpoint_file_dir)

    print(f"Plotting the loss landscape for {file_name}")
    plot_loss_landscape_3d(optimal_parameters=model.parameters(),
                           model=model,
                           evaluate_function=evaluate,
                           n_jobs=25,
                           num_points=50,
                           file_directory="Convolution_CIFAR10_3D_plot")