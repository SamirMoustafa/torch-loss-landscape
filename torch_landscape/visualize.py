...
class PlotlyVisualization(VisualizationBase, ABC):
    ...
    def set_labels(self, fig: go.Figure, title: Optional[str] = None):
        ...
        fig.update_layout(template=self.plotly_template,
                          title={"text": (title if self.options.show_title else ""), "x": 0.5},
                          font_size=10 if self.options.show_axis_labels else 1,
                          scene={
                              ...
                              "zaxis": {"title": (surface_landscape_name if self.options.show_axis_labels else ""),
