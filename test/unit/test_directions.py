...
import torch
from torch import allclose, norm, rand, sign, sort, tensor, cat, dist, stack
...
class DirectionsTest(TestCase):
    ...
    def test_create_svd_directions_from_example(self):
        """
        Tests if the SVD directions are calculated correctly
        (from https://www.vtupulse.com/machine-learning/principal-component-analysis-solved-example/).
        """
        s1 = [tensor([4]), tensor([11])]
        s2 = [tensor([8]), tensor([4])]
        ...
        samples = [s1, s2, s3, s4]
        optimized_parameters_vector = parameters_to_vector(s1)
        b1, b2 = SvdDirections.create_learnable_directions(samples, s1)
        x = [
            (parameters_to_vector(result) - optimized_parameters_vector)
            for result in samples
        ]
        dataset = stack(x).to(dtype=torch.float32)
        V = stack([b1, b2])

        self.assertTrue(dist(dataset, dataset @ V.T @ V) < 1e-5)
